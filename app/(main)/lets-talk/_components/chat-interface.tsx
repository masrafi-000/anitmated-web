...
export function ChatInterface() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      sender: "bot",
      type: "text",
      content: "Hello! Welcome to Ruby Studio support. How can we help you today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newBotMsg: Message = {
        ...
        sender: "bot",
        type: "text", // The main message is text, but it *contains* a form potentially
        content: "Please fill up this form and we will contact you within a few hours.",
        hasForm: true,
  ...
  const handleFormSubmit = (data: FormData, messageId: string) => {
    ...
    setTimeout(() => {
      const thanksMsg: Message = {
        ...
        sender: "bot",
        type: "text",
        content: `Thanks ${data.name}! We have received your details and will contact you soon.`,
        timestamp: new Date(),
      };
              ...
              >
                ...
                {msg.hasForm && !msg.isFormSubmitted && (
                  <div className="mt-4 pt-4 border-t border-border/50">
                    <ChatForm onSubmit={(data) => handleFormSubmit(data, msg.id)} />
                  </div>
                )}
                ...
                {/* Metadata */}
                <div className={cn("text-[10px] mt-1 flex items-center gap-1 opacity-70", msg.sender === 'user' ? 'justify-end text-primary-foreground/80' : 'justify-end text-muted-foreground')}>
